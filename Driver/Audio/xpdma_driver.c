...
#define BUF_SIZE  (1<<20)      // 4 MBytes read/write buffer size
...
static int XdmaInOutMmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    ///0~7 音频通道
    else if ((iLength >= 0x10000 && vma->vm_pgoff < 8) || (iLength >= 0x10000 && iLength < 0x110000 && vma->vm_pgoff >= 8))
    {
        ...
        if (remap_pfn_range(vma,
                           ...
                           xpdev->bufHWAddr[vma->vm_pgoff] >> 12,
                           iLength - 0x10000,
                           vma->vm_page_prot))
        {
    ...
    ///9 收发
    else if (iLength >= 0x110000 && vma->vm_pgoff >= 8)
    {
        if (remap_pfn_range(vma,
                            vma->vm_start,
                            xpdev->bar[0].hdwr_addr >> 12,
        ...
        if (remap_pfn_range(vma,
                           vma->vm_start + 0x110000,
                           xpdev->bufHWAddr[9] >> 12,
                           iLength - 0x110000,
                           vma->vm_page_prot))
        {
            printk(KERN_WARNING"mmap output dma right error\n");
...
static u32 ConfigureDescriptor(XdmaPciDev *xpdev, DMA_DESCRIPTOR *descriptor, u32 length, u32 offset, bool toDevice)
{
    ...
    for (i = 0; i < 1; i++)
    {
        ...
        if (toDevice)
        {
            ...
            srcAddr.QuadPart = xpdev->bufHWAddr[index];
        ...
        else
        {
            ...
            dstAddr.QuadPart = xpdev->bufHWAddr[index];
...
static int XdmaGetResource(XdmaPciDev *xpdev) 
{
    ...
    for (i = 0; i < 10; i++){
        xpdev->buf[i] = dma_alloc_coherent(&xpdev->pdev->dev, BUF_SIZE, &xpdev->bufHWAddr[i], GFP_KERNEL);
        if (NULL == xpdev->buf[i]) {
...
static void XdmaFreeResource(XdmaPciDev *xpdev)
{
    ...
    {
        if (NULL != xpdev->buf[i]) {
            dma_free_coherent(&xpdev->pdev->dev, BUF_SIZE, xpdev->buf[i], xpdev->bufHWAddr[i]);
            xpdev->buf[i] = NULL;
        }
    }
}
...
static int XdmaCreateXcdev(XdmaPciDev *xpdev, enum CDEV_TYPE type)
{
    ...
    switch (type) {
    case AUDIO_A2B:
        ret = kobject_set_name(&xcdev->cdev.kobj, g_devnodeNames[(int)type/4], xpdev->boardName, xpdev->slot);
        break;
    default:
        ret = kobject_set_name(&xcdev->cdev.kobj, g_devnodeNames[(int)type/4], xpdev->boardName, xpdev->slot, (int)type % 4 +1);
        break;
    }
    ...
    xcdev->sys_device = device_create(g_xdmaClass, &xpdev->pdev->dev, xcdev->cdevno,
                                      NULL, g_devnodeNames[(int)type/4], xpdev->boardName, xpdev->slot, (int)type % 4 +1);
    if (!xcdev->sys_device) {
...
static int XdmaProbeOne(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    for (index = AUDIO_IN1; index <= AUDIO_A2B; index++)
    {
        ret = XdmaCreateXcdev(xpdev, index);
        if (ret) {
            XdmaFreeXcdev(&xpdev->xcdev[index]);
            goto unreg_region;
        }
    }
    dev_set_drvdata(&pdev->dev, xpdev);

    mutex_init(&xpdev->lock);
    return 0;
